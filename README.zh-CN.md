...
- **配置以明文传输** — `opencode.json`/`opencode.jsonc`、`AGENTS.md` 和 skills 在 zip 中可读、可 diff。
...
- **无符号链接，无机器特定路径** — link/own 状态以元数据形式记录，导入时重建（无法创建符号链接的 `linked` 域，如在无权限的 Windows 上，会降级为 owned 副本）。opencode 配置中的绝对路径 `{file:...}` 引用会被重写为指向新机器的存储根目录。246 MB 的会话数据库、缓存和 `.bak` 文件永远不会被包含。
