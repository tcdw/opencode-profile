...
- **配置以明文传输** — `opencode.json`/`opencode.jsonc`、`AGENTS.md`、skills，以及实时全局 opencode 配置（`~/.config/opencode/`）和数据（`~/.local/share/opencode/`）在 zip 中可读、可 diff。
...
- **无符号链接，无机器特定路径** — link/own 状态以元数据形式记录，导入时重建（无法创建符号链接的 `linked` 域，如在无权限的 Windows 上，会降级为 owned 副本）。opencode 配置中的绝对路径 `{file:...}` 引用会被重写为指向新机器的存储根目录。246 MB 的会话数据库、缓存、`.bak` 文件，以及已由共享层管理的条目（`skills/`、`auth.json`、`mcp-auth.json`）不会被重复打包到 global 部分。
